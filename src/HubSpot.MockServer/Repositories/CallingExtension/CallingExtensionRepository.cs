...
internal class CallingExtensionRepository
{
    ...
    private JsonElement CreateSettingsObject(string appId, JsonElement body)
    {
    ...
    private JsonElement MergeSettings(JsonElement existing, JsonElement updates, string appId)
    {
    ...
    private JsonElement SetPropertyValue(JsonElement element, string propertyName, object value)
    {
        var dict = JsonSerializer.Deserialize<Dictionary<string, object?>>(element.GetRawText())
            ?? new Dictionary<string, object?>();
        dict[propertyName] = value;
        dict["updatedAt"] = _timeProvider.GetUtcNow();
