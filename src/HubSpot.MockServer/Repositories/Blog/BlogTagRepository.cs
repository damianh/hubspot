...
internal class BlogTagRepository
{
    ...
    public BlogTagRepository(TimeProvider timeProvider) => _timeProvider = timeProvider;

    public BlogTag Create(BlogTag tag)
    {
        tag.Id = _nextId++.ToString();
        tag.Created = _timeProvider.GetUtcNow().UtcDateTime;
        tag.Updated = _timeProvider.GetUtcNow().UtcDateTime;
        _tags[tag.Id] = tag;
        return tag;
    }
    ...
    public BlogTag? Update(string id, BlogTag updatedTag)
    {
        ...
        updatedTag.Id = id;
        updatedTag.Updated = _timeProvider.GetUtcNow().UtcDateTime;
        _tags[id] = updatedTag;
        return updatedTag;
    }
