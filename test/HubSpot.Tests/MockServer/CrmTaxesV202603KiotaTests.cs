...
public sealed class CrmTaxesV202603KiotaTests : IAsyncLifetime
{
    ...
    {
        var result = await _client.Crm.Objects.TwoZeroTwoSixZeroThree["taxes"].PostAsync(
            new SimplePublicObjectInputForCreate
            {
                Properties = new SimplePublicObjectInputForCreate_properties
                {
                    AdditionalData = new Dictionary<string, object> { { "name", "Test Taxes" } }
                }
            });
    ...
    [Fact]
    public async Task Can_list_taxs()
    {
        var result = await _client.Crm.Objects.TwoZeroTwoSixZeroThree["taxes"].GetAsync();
