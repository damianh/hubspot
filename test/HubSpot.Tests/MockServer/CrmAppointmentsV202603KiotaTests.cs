...
public sealed class CrmAppointmentsV202603KiotaTests : IAsyncLifetime
{
    ...
    {
        var result = await _client.Crm.Objects.TwoZeroTwoSixZeroThree["appointments"].PostAsync(
            new SimplePublicObjectInputForCreate
            {
                Properties = new SimplePublicObjectInputForCreate_properties
                {
                    AdditionalData = new Dictionary<string, object> { { "name", "Test Appointment" } }
                }
            });
    ...
    {
        var result = await _client.Crm.Objects.TwoZeroTwoSixZeroThree["appointments"].GetAsync();
