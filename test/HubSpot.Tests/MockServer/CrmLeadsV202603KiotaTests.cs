...
public sealed class CrmLeadsV202603KiotaTests : IAsyncLifetime
{
    ...
    {
        var result = await _client.Crm.Objects.TwoZeroTwoSixZeroThree["leads"].PostAsync(
            new SimplePublicObjectInputForCreate
            {
                Properties = new SimplePublicObjectInputForCreate_properties
                {
                    AdditionalData = new Dictionary<string, object> { { "name", "Test Lead" } }
                }
            });
    ...
    {
        var result = await _client.Crm.Objects.TwoZeroTwoSixZeroThree["leads"].GetAsync();
