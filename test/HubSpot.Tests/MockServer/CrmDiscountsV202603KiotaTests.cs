...
public sealed class CrmDiscountsV202603KiotaTests : IAsyncLifetime
{
    ...
    {
        var result = await _client.Crm.Objects.TwoZeroTwoSixZeroThree["discounts"].PostAsync(
            new SimplePublicObjectInputForCreate
            {
                Properties = new SimplePublicObjectInputForCreate_properties
                {
                    AdditionalData = new Dictionary<string, object> { { "name", "Test Discounts" } }
                }
            });
    ...
    {
        var result = await _client.Crm.Objects.TwoZeroTwoSixZeroThree["discounts"].GetAsync();
