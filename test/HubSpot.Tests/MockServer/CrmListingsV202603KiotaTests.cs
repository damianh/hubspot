...
public sealed class CrmListingsV202603KiotaTests : IAsyncLifetime
{
    ...
    {
        var result = await _client.Crm.Objects.TwoZeroTwoSixZeroThree["listings"].PostAsync(
            new SimplePublicObjectInputForCreate
            {
                Properties = new SimplePublicObjectInputForCreate_properties
                {
                    AdditionalData = new Dictionary<string, object> { { "name", "Test Listings" } }
                }
            });
    ...
    {
        var result = await _client.Crm.Objects.TwoZeroTwoSixZeroThree["listings"].GetAsync();
