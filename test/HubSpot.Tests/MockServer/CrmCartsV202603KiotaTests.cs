...
public sealed class CrmCartsV202603KiotaTests : IAsyncLifetime
{
    ...
    {
        var result = await _client.Crm.Objects.TwoZeroTwoSixZeroThree["carts"].PostAsync(
            new SimplePublicObjectInputForCreate
            {
                Properties = new SimplePublicObjectInputForCreate_properties
                {
                    AdditionalData = new Dictionary<string, object> { { "name", "Test Carts" } }
                }
            });
    ...
    {
        var result = await _client.Crm.Objects.TwoZeroTwoSixZeroThree["carts"].GetAsync();
